...
app.get('/api/gha/overview', verifyStaffToken, async (req, res) => {
  try {
    ...
    // Active = subscription_status is active AND subscription_end is in the future
    ...
    // Expired = had a paid plan but subscription_end is in the past
    var expiredAgents = agents.filter(function(a) {
    ...
    var freeAgents = agents.filter(function(a) {
      return !a.subscription_tier || a.subscription_tier === 'free';
    });

    // Total revenue generated by agents under this GHA
    var totalRevenue = agents.reduce(function(sum, a) {
      return sum + parseFloat(a.subscription_amount || 0);
    }, 0);
    ...
    const [allEarningsResult, inspectionsResult, ghaInfoResult, soldResult, activeResult, confirmedInspResult, pendingInspResult, doneInspResult] = await Promise.all([
      ...
      adminClient
        .from('gha_agents')
        .select('average_rating, total_ratings, gha_code, full_name')
        .eq('id', ghaId)
        .single(),
...
// GET /api/sa/overview
app.get('/api/sa/overview', async (req, res) => {
  try {
    const token = (req.headers.authorization || '').replace('Bearer ', '').trim();
    const { data: session } = await adminClient.from('staff_sessions')
      .select('*').eq('token', token).gt('expires_at', new Date().toISOString())
    ...
    const { data: directAgents } = await adminClient
      .from('profiles')
      .select('id, email, subscription_tier, subscription_status, subscription_end, subscription_amount, gha_id, gha_code')
      .eq('sa_id', session.staff_id)
      .eq('role', 'agent');

    // Also get agents via GHAs
    ...
    if (ghaIds.length > 0) {
      const { data: viaGha } = await adminClient
        .from('profiles')
        .select('id, email, subscription_tier, subscription_status, subscription_end, subscription_amount, gha_id, gha_code')
        .in('gha_id', ghaIds)
        .eq('role', 'agent');
    ...
    // Total subscription revenue = sum of all agent subscription_amounts
    var totalRevenue = agents.reduce(function(sum, a) {
      return sum + parseFloat(a.subscription_amount || 0);
    }, 0);
    ...
    const { count: pendingCount } = await adminClient
      ...
      .select('id', { count: 'exact', head: true })
      .eq('sa_id', session.staff_id)
      .in('status', ['pending', 'pending_sa_review', 'pending_gha_inspection'])
      .eq('role', 'agent');

    // Inspections this month
    const { count: inspConfirmed } = await adminClient
      .from('inspections')
      .select('id', { count: 'exact', head: true })
    ...
    const { count: inspPending } = await adminClient
      .from('inspections')
      .select('id', { count: 'exact', head: true })
      .eq('assigned_by_sa', session.staff_id)
      .in('status', ['pending', 'assigned'])
...
// ============ ADMIN EARNINGS - shows all subscriptions and commissions ============
app.get('/api/admin/earnings', async (req, res) => {
  try {
    ...
    // Get GHA earnings with GHA details
    const { data: ghaEarnings } = await adminClient
      .from('gha_earnings')
      .select('gha_id, agent_id, agent_email, commission_amount, commission_rate, subscription_amount, payment_reference, is_paid, paid_at, created_at')
      .eq('month_year', month)
      .order('created_at', { ascending: false });
    ...
    const { data: saEarnings } = await adminClient
      .from('sa_earnings')
      .select('sa_id, agent_id, commission_amount, commission_rate, subscription_amount, payment_reference, is_paid, paid_at, created_at')
      .eq('month_year', month)
      .order('created_at', { ascending: false });
    ...
    var agentIds = [...new Set([
      ...(ghaEarnings || []).map(function(e) { return e.agent_id; }),
      ...(saEarnings || []).map(function(e) { return e.agent_id; }),
    ].filter(Boolean))];
    var agentProfileMap = {};
    if (agentIds.length > 0) {
      const { data: agentProfs } = await adminClient
        .from('profiles')
    ...
    // Build per-payment earnings list
    var earningsMap = {};
    (ghaEarnings || []).forEach(function(ge) {
      var ref = ge.payment_reference || ('gha_' + ge.agent_id + '_' + ge.created_at);
      var gha = ghaMap[ge.gha_id] || {};
      var agent = agentProfileMap[ge.agent_id] || {};
      earningsMap[ref] = earningsMap[ref] || {
        payment_reference: ref,
        agent_id: ge.agent_id,
        agent_email: ge.agent_email || agent.email || '—',
        agent_name: agent.full_name || null,
        subscription_tier: agent.subscription_tier || null,
        ...
        sa_name: null,
        sa_commission: 0,
        is_paid_gha: ge.is_paid,
        is_paid_sa: false,
        created_at: ge.created_at,
        plan_type: ref.includes('unlimited') ? 'unlimited' : 'subscription',
      };
      earningsMap[ref].gha_commission += parseFloat(ge.commission_amount || 0);
    });

    (saEarnings || []).forEach(function(se) {
      var ref = se.payment_reference || ('sa_' + se.agent_id + '_' + se.created_at);
      var sa = saMap[se.sa_id] || {};
      var agent = agentProfileMap[se.agent_id] || {};
      if (earningsMap[ref]) {
        earningsMap[ref].sa_code = sa.sa_code;
        earningsMap[ref].sa_name = sa.full_name;
        earningsMap[ref].sa_commission += parseFloat(se.commission_amount || 0);
        earningsMap[ref].is_paid_sa = se.is_paid;
      } else {
        earningsMap[ref] = {
          payment_reference: ref,
    ...
    var unpaidGha = earningsList.filter(function(e) { return !e.is_paid_gha; }).reduce(function(sum, e) { return sum + e.gha_commission; }, 0);
    var unpaidSa = earningsList.filter(function(e) { return !e.is_paid_sa; }).reduce(function(sum, e) { return sum + e.sa_commission; }, 0);
    var agentCount = new Set(earningsList.map(function(e) { return e.agent_id; })).size;

    console.log('Admin earnings', month, '| rows:', earningsList.length, '| revenue:', totalRevenue, '| gha:', totalGha, '| sa:', totalSa);
    ...
    res.json({
      ...
      summary: {
        ...
        unpaid_gha: unpaidGha,
        unpaid_sa: unpaidSa,
        total_commission: totalGha + totalSa,
        agent_count: agentCount,
      },
    });
  } catch(err) {
...
app.post('/api/sa/confirm-inspection', verifyStaffToken, async (req, res) => {
  try {
    ...
    // Fetch by id only - assigned_by_sa may not match this SA's staff_id even
    // when the inspection's GHA reports to this SA, so ownership is checked below.
    const { data: inspection } = await serviceClient
      .from('inspections').select('*').eq('id', inspection_id).single();
    if (!inspection) return res.status(404).json({ error: 'Inspection not found' });

    // Check SA ownership - either directly assigned OR GHA belongs to this SA
    var isAuthorized = inspection.assigned_by_sa === saId;

    if (!isAuthorized && inspection.gha_id) {
      const { data: gha } = await serviceClient
        .from('gha_agents')
        .select('sa_id')
        .eq('id', inspection.gha_id)
        .single();
      isAuthorized = gha?.sa_id === saId;
    }
    ...
    console.log('Inspection confirmed:', inspection_id, '| SA:', saId, '| GHA:', inspection.gha_id);
...
// Hourly earnings recovery job - backfills earnings only for subscription
// payments that actually landed in the current billing month but whose webhook
// dropped the earnings row. See the safety guards note below.
...
app.post('/api/flutterwave/webhook', async (req, res) => {
  ...
  try {
    ...
    if (status === 'successful' && paymentType === 'subscription') {
      ...
      console.log('=== SUBSCRIPTION WEBHOOK START ===');
      console.log('agentId:', agentId, '| tier:', tier, '| amount:', amount);
      if (!agentId || !tier) {
        console.error('CRITICAL: subscription webhook missing agentId or tier - earnings cannot be created. meta:', JSON.stringify(verifiedMeta));
      }
...
app.get('/api/sa/monthly-history', async (req, res) => {
  try {
    ...
    // Get ALL SA earnings for this month
    const { data: allEarnings } = await adminClient
      .from('sa_earnings')
    ...
    // Get agent emails for display
    var agentIds = [...new Set((allEarnings || []).map(function(e) { return e.agent_id; }).filter(Boolean))];
    var agentMap = {};
    if (agentIds.length > 0) {
      const { data: agentProfs } = await adminClient
        .from('profiles')
        .select('id, email, full_name, subscription_tier')
        .in('id', agentIds);
      (agentProfs || []).forEach(function(p) { agentMap[p.id] = p; });
    }

    // Group by agent
    var agentEarningsMap = {};
    (allEarnings || []).forEach(function(e) {
      var key = e.agent_id;
      if (!agentEarningsMap[key]) {
        var prof = agentMap[e.agent_id] || {};
        agentEarningsMap[key] = {
          agent_id: e.agent_id,
          agent_email: prof.email || e.agent_id,
          agent_name: prof.full_name || null,
          subscription_tier: prof.subscription_tier || null,
      ...
      agentEarningsMap[key].total_commission += parseFloat(e.commission_amount || 0);
      agentEarningsMap[key].total_subscription += parseFloat(e.subscription_amount || 0);
      agentEarningsMap[key].payments.push(e);
      if (!e.is_paid) agentEarningsMap[key].is_fully_paid = false;
    });
    ...
    var unpaidTotal = (allEarnings || [])
      .filter(function(e) { return !e.is_paid; })
      .reduce(function(sum, e) { return sum + parseFloat(e.commission_amount || 0); }, 0);
    var totalRevenue = (allEarnings || [])
      .reduce(function(sum, e) { return sum + parseFloat(e.subscription_amount || 0); }, 0);

    console.log('SA monthly history', month, '| rows:', (allEarnings||[]).length, '| unpaid:', unpaidTotal, '| paid:', paidTotal);

    // Build agent_snapshots in the shape SA frontend expects
    var agentSnapshots = Object.values(agentEarningsMap).map(function(agent) {
      return {
        ...
        subscription_amount: agent.total_subscription,
        commission_amount: agent.total_commission,
        commission_rate: agent.payments[0]?.commission_rate || 0,
        is_paid: agent.is_fully_paid,
        paid_at: agent.payments.find(function(p) { return p.paid_at; })?.paid_at || null,
        payment_reference: agent.payments[0]?.payment_reference || null,
        payments: agent.payments,
        month_year: month,
      };
    });

    // Get available months from earnings history
    const { data: availableMonthsData } = await adminClient
      .from('sa_earnings')
      .select('month_year')
      .eq('sa_id', session.staff_id)
      .order('month_year', { ascending: false });
...
app.get('/api/gha/monthly-history', async (req, res) => {
  try {
    ...
    // Get ALL GHA earnings for this month
    const { data: allGhaEarnings } = await adminClient
      .from('gha_earnings')
    ...
    // Get agent details
    var ghaAgentIds = [...new Set((allGhaEarnings || []).map(function(e) { return e.agent_id; }).filter(Boolean))];
    var ghaAgentMap = {};
    if (ghaAgentIds.length > 0) {
      const { data: ghaAgentProfs } = await adminClient
        .from('profiles')
        .select('id, email, full_name, subscription_tier')
        .in('id', ghaAgentIds);
      (ghaAgentProfs || []).forEach(function(p) { ghaAgentMap[p.id] = p; });
    }

    var ghaAgentSnapshots = (allGhaEarnings || []).map(function(e) {
      var prof = ghaAgentMap[e.agent_id] || {};
      return {
        agent_id: e.agent_id,
        agent_email: e.agent_email || prof.email || '—',
        email: e.agent_email || prof.email || '—',
    ...
    var ghaPaid = (allGhaEarnings || []).filter(function(e) { return e.is_paid; }).reduce(function(sum, e) { return sum + parseFloat(e.commission_amount || 0); }, 0);
    var ghaUnpaid = (allGhaEarnings || []).filter(function(e) { return !e.is_paid; }).reduce(function(sum, e) { return sum + parseFloat(e.commission_amount || 0); }, 0);
    var ghaRevenue = (allGhaEarnings || []).reduce(function(sum, e) { return sum + parseFloat(e.subscription_amount || 0); }, 0);

    // Available months
    const { data: ghaMonthsData } = await adminClient
      .from('gha_earnings')
      .select('month_year')
      .eq('gha_id', session.staff_id)
      .order('month_year', { ascending: false });
    var ghaAvailableMonths = [...new Set((ghaMonthsData || []).map(function(r) { return r.month_year; }))];

    console.log('GHA monthly history', month, '| earnings rows:', (allGhaEarnings || []).length, '| total commission:', ghaPaid + ghaUnpaid);

    res.json({
      month,
      agent_snapshots: ghaAgentSnapshots,
      available_months: ghaAvailableMonths,
      totals: {
        total_revenue: ghaRevenue,
        total_commission: ghaPaid + ghaUnpaid,
        pending_commission: ghaUnpaid,
        paid_commission: ghaPaid,
        agent_count: ghaAgentSnapshots.length,
      },
      paid_commission: ghaPaid,
      pending_commission: ghaUnpaid,
      total_commission: ghaPaid + ghaUnpaid,
...
// gha_earnings/sa_earnings rows carry no plan-type column, so there is no way to
// tell from an earnings row alone whether it came from a premium, agency, or
// unlimited plan payment. This derives an approximate unlimited-plan share of a
// commission total by joining each earning's agent_id to profiles.is_unlimited —
// it reflects the agent's CURRENT unlimited status, not necessarily their plan
// at the time the earning was recorded.
async function sumCommissionForUnlimitedAgents(earnings) {
  var agentIds = [...new Set((earnings || []).map(function(e) { return e.agent_id; }).filter(Boolean))];
  if (agentIds.length === 0) return 0;

  const { data: agentFlags } = await adminClient
    .from('profiles')
    .select('id, is_unlimited')
    .in('id', agentIds);

  var unlimitedAgentIds = new Set((agentFlags || []).filter(function(a) { return a.is_unlimited; }).map(function(a) { return a.id; }));

  return (earnings || []).reduce(function(sum, e) {
    return sum + (unlimitedAgentIds.has(e.agent_id) ? (parseFloat(e.commission_amount) || 0) : 0);
  }, 0);
}
...
app.get('/api/admin/staff-payments', async (req, res) => {
  try {
    ...
    const { data: ghas } = await adminClient
      .from('gha_agents')
      .select('id, gha_code, full_name, location, bank_name, account_number, account_name, bank_code, commission_rate, commission_rate_override, sa_id');
    ...
    const { data: sas } = await adminClient
      .from('service_agents')
      .select('id, sa_code, full_name, location, bank_name, account_number, account_name, bank_code, commission_rate, commission_rate_override, whatsapp');
...
// POST /api/admin/fix-unlimited-earnings - one-time manual fix to create GHA/SA
// earnings rows for an unlimited-plan agent whose payment didn't trigger them.
app.post('/api/admin/fix-unlimited-earnings', async (req, res) => {
  try {
    const admin = await verifyAdminToken(req);
    if (!admin) return res.status(401).json({ error: 'Unauthorized' });
    ...
    const { data: agentProf } = await adminClient
      .from('profiles')
      .select('id, email, gha_id, sa_id, subscription_amount, is_unlimited')
      .eq('email', agent_email)
      .single();

    if (!agentProf) return res.status(404).json({ error: 'Agent not found' });
    if (!agentProf.is_unlimited) return res.status(400).json({ error: 'Agent is not on unlimited plan' });
    ...
    if (agentProf.gha_id) {
      ...
      const { error: ghaErr } = await adminClient.from('gha_earnings').upsert([{
        gha_id: agentProf.gha_id,
        agent_id: agentProf.id,
        agent_email: agent_email,
        subscription_amount: unlimitedAmount,
        commission_rate: ghaRate,
        commission_amount: ghaCommission,
      ...
      results.push('No GHA linked to agent');
    }

    if (agentProf.sa_id) {
      var saRate = await getStaffCommissionRate(agentProf.sa_id, 'SA');
      var saCommission = Math.round(unlimitedAmount * (saRate / 100));
      const { error: saErr } = await adminClient.from('sa_earnings').upsert([{
        sa_id: agentProf.sa_id,
        gha_id: agentProf.gha_id || null,
        agent_id: agentProf.id,
        subscription_amount: unlimitedAmount,
        commission_rate: saRate,
        commission_amount: saCommission,
      ...
      results.push('No SA linked to agent');
    }
